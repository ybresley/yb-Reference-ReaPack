...
-- v3 -> v4: categories become one flat list. Sounds that were filed in a
-- sub-category move to its parent category, then the sub-category records are
-- removed. v4 introduced the new six-colour palette; v5 below repairs the
-- initial assignment so every upgraded library receives its intended cycle.
migrations[3] = function(data)
  local parent_by_id = {}
  if type(data.categories) == "table" then
    for _, c in ipairs(data.categories) do
      if type(c) == "table" and c.parent ~= nil then
        parent_by_id[c.id] = c.parent
      end
    end
  end
  ...
  if type(data.sounds) == "table" then
    for _, s in ipairs(data.sounds) do
      if type(s) == "table" then
        local parent = parent_by_id[s.subcategory] or parent_by_id[s.category]
        if parent ~= nil then s.category = parent end
        s.subcategory = nil
      end
    end
  end
...
-- v4 -> v5: the first flat-category build translated each old colour by hue.
-- That kept the old sequence and often used only five of the six new colours,
-- while the approved design showed the category list cycling through all six
-- in order. Reassign once by list position; choices made after this migration
-- remain stable and new categories continue the same cycle.
migrations[4] = function(data)
  if type(data.categories) == "table" then
    for i, c in ipairs(data.categories) do
      if type(c) == "table" then
        c.color = categories.PALETTE[((i - 1) % #categories.PALETTE) + 1]
      end
    end
  end
  data.version = 5
...
function schema.validate(data)
  ...
  for _, c in ipairs(data.categories) do
    if c.parent ~= nil then
      error("a category still has a parent (file is corrupt or was hand-edited)")
    end
  end
  for _, s in ipairs(data.sounds) do
    if s.subcategory ~= nil then
      error("a sound still has a sub-category (file is corrupt or was hand-edited)")
    end
