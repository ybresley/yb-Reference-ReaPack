...
-- One duplicate rule for adding and editing presets. `except_index` lets an
-- edited row keep its current target without mistaking itself for a duplicate.
function match.preset_exists(list, unit, value, except_index)
  if type(list) ~= "table" then return false end
  for i, p in ipairs(list) do
    if i ~= except_index and p.unit == unit and p.value == value then return true end
  end
  return false
end
