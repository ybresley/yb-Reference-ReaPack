...
function store.decode(text)
  ...
  local stored_version = result.version
  local data = schema.migrate(result)
  return data, stored_version ~= data.version
end
...
-- Load a library from disk. A successful migration is saved immediately so the
-- on-disk file and the in-memory shape cannot drift apart or repeat repairs on
-- every launch. Atomic save keeps the previous file intact if persistence fails.
-- Returns the current table, or raises if the file is missing, corrupt, or the
-- migrated file cannot be saved. The caller decides whether "missing" means a
-- new library may be created.
function store.load(path)
  local text = read_file(path)
  if text == nil then
    error("library file not found: " .. tostring(path))
  end
  local data, migrated = store.decode(text)
  if migrated then store.save(path, data) end
  return data
end
