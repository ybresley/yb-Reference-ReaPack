...
--
-- Pure Lua. Filing stores one category id on each sound.
...
--   { scope = "category", id = "c1" }    sounds filed in c1
--   { scope = "categories", ids = {...} } the union of every selected category
local function in_view(s, view)
  ...
  if scope == "categories" then
    local ids = view.ids or {}
    return ids[s.category] == true
  end
