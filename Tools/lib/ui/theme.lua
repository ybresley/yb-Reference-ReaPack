...
local T = {
  ...
  SCROLL_THUMB     = 0xE4E4E41E, -- = FILL_SECONDARY: quiet at rest; the wider pill stays visible
...
local BASE = {
  ...
  -- floats over the rows' right end. The digits stop at 8 and the sidebar thumb
  -- hugs the outer edge, preserving a 2px gap between them.
  --
  ...
  -- between 13px caps lines. The row's own vertical padding gives the text room
  -- without putting bare background between adjacent highlights.
  SB_ROW_GAP = 2,
  SB_ROW_PAD_Y = 2,
  -- Compact category colour row in the right-click menu. The 2px ring sits
  -- outside the selected swatch without touching its neighbours.
  CATEGORY_SWATCH_SIZE = 16,
  CATEGORY_SWATCH_GAP = 5,
  CATEGORY_SWATCH_RING_GAP = 2,
  ...
  FIELD_W   = 176, -- standard single-line text-input width
  ...
  GROUP_FS  = 12,  -- small text size for subordinate metadata
