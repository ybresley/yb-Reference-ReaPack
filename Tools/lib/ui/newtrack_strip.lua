...
-- This overlay belongs visually to REAPER rather than the tool, so its text
-- size follows the fixed 20px arrange band instead of the app's compact font
-- scale. Fifteen pixels leaves clear air above and below without reading as
-- fine print.
local STRIP_FONT_SIZE = 15
local UNIT = "New Track / "

-- Resize the current system font without creating a second font resource.
-- Older ReaImGui builds that lack the resize form keep the normal body size.
local strip_font_ok
local function push_strip_font(ctx)
  if strip_font_ok == nil then
    strip_font_ok = pcall(reaper.ImGui_PushFont, ctx, nil, STRIP_FONT_SIZE)
    return strip_font_ok
  end
  if strip_font_ok then
    reaper.ImGui_PushFont(ctx, nil, STRIP_FONT_SIZE)
    return true
  end
  return false
end
...
function newtrack_strip.draw(ctx, zone)
  ...
  local font_pushed = push_strip_font(ctx)
