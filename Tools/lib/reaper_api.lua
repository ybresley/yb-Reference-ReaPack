...
-- Publish whether this ReaScript action is running. The machine-local Dev Hub
-- reads the action's official toggle state so it can disable every launcher
-- while any Main/DEV copy is alive. This is session state only; a crash cannot
-- leave a persistent lock behind. `SetToggleCommandState` is documented for
-- ReaScripts, and RefreshToolbar2 makes the new state visible immediately.
function reaper_api.mark_action_running(cmd_id, running)
  if not cmd_id or cmd_id == 0 then return false end
  if not reaper.SetToggleCommandState then return false end
  -- REAPER documents this setter as returning no value.
  reaper.SetToggleCommandState(0, cmd_id, running and 1 or 0)
  if reaper.RefreshToolbar2 then reaper.RefreshToolbar2(0, cmd_id) end
  return true
end
